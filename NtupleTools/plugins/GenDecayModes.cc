...
class GenDecayModes : public NtupleFillerBase {
 public:
    ...
    src_(iConfig.getParameter<edm::InputTag>("src"))
	{
	   weight=-1;
	    ...
	    foundDPM=0,foundD0=0, foundD0Star=0;
	    identifiedCDecay=0;
 ...
 protected:
  ...
  bool foundDPM,foundD0, foundD0Star;
...
void GenDecayModes::fill(const edm::Event& iEvent,const edm::EventSetup& iSetup){
    ...
    foundDPM=false,foundD0=false, foundD0Star=false;
  ...
  if(iEvent.getByLabel(src_, pGenPart)){
	...
	if(identifiedCDecay&&verbose_){
		  cout<<"FOUND DPM? "<<foundDPM<<endl;  if(foundDPM) weight=1.3;
		  cout<<"FOUND D0? "<<foundD0<<endl;
		  cout<<"FOUND D0STAR? "<<foundD0Star<<endl;	if(foundD0Star) weight=0.8;
	}
